  ...
  <form action="" method="get" id="filterForm">
    ...
    <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="md:flex items-center gap-2">
        ...
        <a href="{{ route('usuarios.create') }}" class="bg-blue-600 text-white mb-3 rounded-lg px-4 py-2 flex items-center">
  ...
  @else
    ...
    <div class="rounded-xl border border-gray-200 dark:border-[#1F1F23] overflow-hidden">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 dark:bg-[#1F1F23] border-b border-gray-200 dark:border-[#2B2B30]">
              ...
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Nombre de usuario</th>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Email</th>
              <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">Fecha de creación</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-[#2B2B30] bg-white dark:bg-[#0F0F12]">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-[#1F1F23]/50 transition-colors duration-150">
              ...
              <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $usuario->id }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $usuario->name }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $usuario->username }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $usuario->email }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $usuario->created_at }}</td>
      ...
      <div class="bg-white dark:bg-[#0F0F12] px-4 py-3 border-t border-gray-200 dark:border-[#2B2B30] text-sm text-gray-500 dark:text-gray-400">
        {{ $usuarios->links() }}
        {{-- <span class="text-gray-700 dark:text-gray-400">Showing {{ $usuarios->firstItem() }} to {{ $usuarios->lastItem() }} of {{ $usuarios->total() }} entries</span> --}}
      </div>
    </div>
  @endif

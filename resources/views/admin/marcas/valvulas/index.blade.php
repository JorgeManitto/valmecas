  ...
  <form action="" method="get" id="filterForm">
    ...
    <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <a href="{{ route('marcas.valvulas.create') }}" class="bg-blue-600 text-white rounded-lg px-4 py-2 flex items-center">
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-[#2B2B30] bg-white dark:bg-[#0F0F12]">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-[#1F1F23]/50 transition-colors duration-150">
              <td class="px-4 py-3 text-yellow-900 dark:text-yellow-100">{{ $marca->nombre }}</td>
              <td class="px-4 py-3">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                  {{ $marca->estado === 'activo' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' }}">
                  {{ $marca->estado }}
                </span>
              </td>
              ...
              <td class="px-4 py-3 text-right relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false"
                  ...
                  <a href="{{ route('marcas.valvulas.edit', $marca->id) }}" 
                     class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#2B2B30]">
                    Editar
                  </a>
                  <form action="{{ route('marcas.valvulas.destroy', $marca->id) }}" method="POST" 
